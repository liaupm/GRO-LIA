...
GenCell::GenCell(std::string name, int state, int type, GenOperon* parent, GenPlasmid* enviroment_plasmid,float time_act,float time_deact){
    this->name = name;
...
GenCell::GenCell(std::string name, int type, GenOperon* parent, GenPlasmid* enviroment_plasmid,float time_act,float time_deact){
    this->name = name;
    this->state=0;
    this->enviroment_plasmid = enviroment_plasmid;
    this->type=type;
...
GenCell::GenCell(std::string name, int state, std::string target, int type, GenOperon* parent, GenPlasmid* enviroment_plasmid,float time_act,float time_deact){
    this->name = name;
    this->enviroment_plasmid = enviroment_plasmid;
    this->state=state;
    this->target = target;
...
GenCell::GenCell(std::string name, std::string target, int type, GenOperon* parent, GenPlasmid* enviroment_plasmid,float time_act,float time_deact){
    this->name = name;
    this->enviroment_plasmid = enviroment_plasmid;
    this->state=0; //-1?
    this->target = target;
...
void GenCell::erase(){
	if(!this->isFree()){
		this->setFree();
		if(this->state == 1){
			if (this->enviroment_plasmid != NULL){
				this->enviroment_plasmid->getOperon((unsigned int)0)->insertGens(this);
			}
		}
	}
...
void GenCell::setTime(float currentTime){
	...
	if(((this->parent)->getPromoter()->check_gates() == 0 )){
		this->time=rng->normal_float(time_deg, sigmaDeact)+currentTime;
	}
}
